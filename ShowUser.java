 ...
 * ShowUser.java (GUI)
 * 
 * The following class is a scene that shows a certain users profile.
 * Depending on the type of the user selected. The details shown will change
 * 
 * For example, a dependents list will only be available to Adult type users
 * On the other hand, a siblings list will only be available to YoungChild type users.
 * 
 */
...
public class ShowUser implements GUIInterface {
	...
	// the following are methods to get the connections of the current user selected
	...
	// overidden method to initialise the scene
	public void initScene() {
		// change header depending on the type of user
		...
		// condition of what age to show in the profile
		...
		// add a spouse field is the user is an adult
		if ( MiniNet.currentUser instanceof Adult ) {
		...
		// add parents field if it is a child user
		} else if ( MiniNet.currentUser instanceof Child) {
		...
		// add parents field if it is a young child user
		} else if ( MiniNet.currentUser instanceof YoungChild) {
	...
	//  this method instantiates the list boxes to show the relationships the current users has
