 ...
 * created: May 5, 2018
 * 
 * AddChildUser.java (GUI)
 * 
 * This scene is only instantiated when the user desires to create a child user profile
 * It will asked the user to choose between a possible list of parents. 
 * It will also adjust the parents if the adult user already has a partner. 
 * 
 */
...
public class AddChildUser implements GUIInterface {
	...
	// add child user constructor
	public AddChildUser (Stage primaryStage, ArrayList<String> childDetails) {
		this.primaryStage = primaryStage;
		this.childDetails = childDetails;
	}
	
	// overriden method to initialise the scene
	public void initScene() {
		// change header depending on the type of profile being created
	...
	// validating the input of the adult users specified to be parents
	public void validateInput() throws NotToBeCoupledException, Exception {
		...
		// the parents fields must be proper
		// 1. if an adult already has a partner, cannot set another adult to be the partner
	...
	// method to check if an existing partner exists
	...
	// method to display the childuser
		...
		createChildUserBtn.setOnAction(new EventHandler<ActionEvent>() {
			public void handle(ActionEvent e) {
				try {
					...
					// create relationships instances betwwen users
