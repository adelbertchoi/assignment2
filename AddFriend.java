 ...
 * AddFriend.java (GUI)
 * 
 * A scene that is called when the users desires to add a friend. 
 * This scene adjusts the possible users shown that can be added to a user.
 * For example, if current user is an adult, then the possible users shown in the
 * list in this scene are only users that can be added by an adult user. 
 * 
 */
...
class AddFriend implements EventHandler<ActionEvent>, GUIInterface {
	...
	// method to get possible friends that can be addded by user
	// this method calls the corresponding methods to initialise the list of users a user can add as a friend
	...
	// initialise the possible friends list view
